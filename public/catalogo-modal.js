...
(function(){
  ...
  function ensureStyles(){
    ...
    style.textContent =
      ...
      '.catalogo-modal{background:#fff;border-radius:12px;max-width:380px;width:100%;' +
      'box-shadow:0 20px 50px rgba(0,0,0,0.25);padding:20px 22px;' +
      ...
      '.catalogo-modal h2{margin:0 0 6px;font-size:15px;font-weight:700;color:#1B3A5C;' +
      'padding-right:24px;}' +
      '.catalogo-modal-mensaje{margin:0 0 16px;font-size:13px;line-height:1.45;color:#5C7480;}' +
      '.catalogo-modal-acciones{display:flex;flex-direction:column;gap:10px;}' +
      // Dos opciones (solo este / todos): botones lado a lado, cortos y parejos.
      '.catalogo-modal-choices{display:flex;gap:10px;}' +
      '.catalogo-modal-choices .catalogo-modal-btn{flex:1;}' +
      '.catalogo-modal-btn{width:100%;border-radius:8px;padding:12px 12px;font-size:13.5px;' +
  ...
  window.mostrarModalCatalogo = function(opts){
    ...
    return new Promise(function(resolve){
      ...
      var mensajeHtml = opts.mensaje ? '<p class="catalogo-modal-mensaje">' + escHtml(opts.mensaje) + '</p>' : '';

      var accionesHtml;
      if (opts.botonSecundario) {
        // Dos opciones: fila compacta [Solo este] [Todos]. El ✕ de arriba cierra/cancela.
        accionesHtml =
          '<div class="catalogo-modal-choices">' +
        ...
        // Una sola opción (caso losetas): botón primario a ancho completo + "Cancelar".
        accionesHtml =

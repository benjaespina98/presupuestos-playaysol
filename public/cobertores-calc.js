(function(){
/* ---------------- STORAGE KEYS ---------------- */
const K_CATALOG = 'presupuesto-cobertor-catalogo-opcionales';
const K_LEGAL   = 'presupuesto-cobertor-texto-legal';
const K_FOOTER  = 'presupuesto-cobertor-footer-empresa';
const K_FOTOS_OPC = 'presupuesto-cobertor-fotos-por-opcional'; // metadata liviana; los bytes van en IndexedDB
const K_PRECIOS = 'presupuesto-cobertor-precios-base'; // precios base compartidos por todo el equipo (no por presupuesto)

/* ---------------- DEFAULTS ---------------- */
const defaultOptionales = [
  {id:cid(), slug:'soga_sujecion_reforzada', desc:'Soga y accesorios de sujeción reforzados', price:null, included:false},
  {id:cid(), slug:'funda_protectora_invierno', desc:'Funda protectora para guardado del cobertor en invierno', price:null, included:false},
];

const defaultLegal = `COBERTOR PARA PISCINA:

El cobertor se realiza a medida, de acuerdo a los metros y al formato de la pileta.

Características:

1. - Lona microperforada, permite el paso del agua de lluvia y evita la acumulación de hojas y suciedad.

2. - Viene con ojales reforzados en todo el perímetro para pasar la soga de sujeción.

3. - El precio detallado corresponde solamente a la lona (sin instalación). La instalación es un servicio aparte, a cargo de nuestro equipo.

El valor por m² varía según la superficie total a cubrir. La instalación es un servicio opcional y se cotiza aparte.
...
const DEFAULT_FOTOS_GENERALES_SEED = [
  "/seeds/general-1-adad76e0.jpg",
  "/seeds/general-2-84060e27.jpg",
  "/seeds/general-3-0ac31460.jpg"
];
...
let state = {
  ...
  validez: '7',
  subtotal: 0,
  largo: 0, ancho: 0, adicionalM2: 0,
  precioMenos15: 10903,
  precioMas15: 9902,
  precioInstalacion: 100000,
  modoPrecio: 'ambos', // 'sin' | 'con' | 'ambos'
  items: [], // adicionales que se suman a AMBOS totales (sin y con instalación)
  opcionales: [],
  legal: defaultLegal,
...
function openFotosDB(){
  ...
  _dbPromise = new Promise((resolve, reject)=>{
    const req = indexedDB.open('PlayaSolCobertoresDB', 1);
    req.onupgradeneeded = ()=>{
...
function loadCatalog(){
  ...
  try{
    const raw4 = localStorage.getItem(K_PRECIOS);
    const precios = raw4 ? JSON.parse(raw4) : null;
    state.precioMenos15 = precios && precios.precioMenos15!=null ? precios.precioMenos15 : 10903;
    state.precioMas15 = precios && precios.precioMas15!=null ? precios.precioMas15 : 9902;
    state.precioInstalacion = precios && precios.precioInstalacion!=null ? precios.precioInstalacion : 100000;
  }catch(e){
    state.precioMenos15 = 10903; state.precioMas15 = 9902; state.precioInstalacion = 100000;
  }
}

function savePreciosBase(){
  try{
    localStorage.setItem(K_PRECIOS, JSON.stringify({
      precioMenos15: state.precioMenos15, precioMas15: state.precioMas15, precioInstalacion: state.precioInstalacion
    }));
    flashPreciosSaved();
  }catch(e){ console.error('No se pudieron guardar los precios base', e); }
}
let flashPreciosTimeout;
function flashPreciosSaved(){
  const el = document.getElementById('precios-base-flash');
  if(!el) return;
  el.textContent = 'Precio base guardado ✓ (para todo el equipo)';
  clearTimeout(flashPreciosTimeout);
  flashPreciosTimeout = setTimeout(()=>{ el.textContent=''; }, 2000);
}
...
// campo suelto del state, ej. precioMenos15/precioMas15/precioInstalacion).
async function confirmarCambioCatalogoPrecioBase(inputEl, clave, etiqueta){
  const valorInicial = inputEl.dataset.valorInicial ?? '';
  const valorNuevo = inputEl.value;
...
function quoteToPlainState(){
  return {
    ...
    largo: state.largo, ancho: state.ancho, adicionalM2: state.adicionalM2,
...
async function aplicarPresupuestoAlState(q){
  ...
  state.largo=q.largo||0; state.ancho=q.ancho||0; state.adicionalM2=q.adicionalM2||0;
  // precioMenos15/precioMas15/precioInstalacion ya NO se guardan por presupuesto: son precio base compartido (K_PRECIOS)
  state.modoPrecio = q.modoPrecio || 'ambos';
...
async function newQuote(){
  ...
  state.subtotal = 0;
  state.largo = 0; state.ancho = 0; state.adicionalM2 = 0;
  // precioMenos15/precioMas15/precioInstalacion quedan como están (precio base compartido)
  state.modoPrecio = 'ambos';
  state.items = [];
...
function renderForm(){
  ...
  document.getElementById('f-adicional-m2').value = state.adicionalM2 || '';
  document.getElementById('f-m2').value = (Number(state.largo||0) * Number(state.ancho||0) + Number(state.adicionalM2||0)).toLocaleString('es-AR', {maximumFractionDigits:2});
  document.getElementById('f-precio-menos15').value = state.precioMenos15;
  document.getElementById('f-precio-menos15').dataset.valorInicial = state.precioMenos15;
  document.getElementById('f-precio-mas15').value = state.precioMas15;
  document.getElementById('f-precio-mas15').dataset.valorInicial = state.precioMas15;
  document.getElementById('f-precio-instalacion').value = state.precioInstalacion;
  document.getElementById('f-precio-instalacion').dataset.valorInicial = state.precioInstalacion;
  document.getElementById('f-modo-precio').value = state.modoPrecio;
...
function buildDocumentBody({ forExport=false, photoSrc } = {}){
  ...
  const adicionalM2 = Number(state.adicionalM2||0);
  const espejoM2 = largo * ancho;
  const m2 = espejoM2 + adicionalM2;
  const precioAuto = m2 > 15 ? Number(state.precioMas15||0) : Number(state.precioMenos15||0);
  const extras = state.items.reduce((s,i)=>s+Number(i.price||0),0);
  const baseSin = m2 * precioAuto;
  const totalSin = baseSin + extras;
  const totalCon = baseSin + Number(state.precioInstalacion||0) + extras;
  const incluidosOpt = state.opcionales.filter(o=>o.included===true);

  let html = `<div class="doc-title">Presupuesto de cobertor para piscina</div>`;
  const medidasParts = [`${largo.toLocaleString('es-AR')} m largo × ${ancho.toLocaleString('es-AR')} m ancho = ${espejoM2.toLocaleString('es-AR',{maximumFractionDigits:2})} m²`];
  if(adicionalM2){ medidasParts.push(`Adicional: ${adicionalM2.toLocaleString('es-AR',{maximumFractionDigits:2})} m²`); }
  ...
  html += `<div class="meta-line" style="grid-column:1/-1;"><b>Medidas:</b> ${medidasParts.join(' + ')} = ${m2.toLocaleString('es-AR',{maximumFractionDigits:2})} m² a cubrir</div>`;
  html += `</div>`;
  ...
  const totalRows = [];
  if(state.modoPrecio === 'sin'){
    totalRows.push({ descHtml:'TOTAL', priceHtml: fmt(totalSin), cls:'total' });
  } else if(state.modoPrecio === 'con'){
    totalRows.push({ descHtml:'TOTAL (incluye instalación)', priceHtml: fmt(totalCon), cls:'total' });
  } else {
    totalRows.push({ descHtml:'TOTAL SIN INSTALACIÓN', priceHtml: fmt(totalSin), cls:'total' });
    totalRows.push({ descHtml:'TOTAL CON INSTALACIÓN', priceHtml: fmt(totalCon), cls:'total' });
  }
  ...
  if(incluidosOpt.length){
    ...
    incluidosOpt.forEach(op=>{
      const priceHtml = (op.price===null||op.price===undefined) ? 'No incluye' : fmt(op.price);
...
document.getElementById('f-adicional-m2').addEventListener('input', e=>{
  state.adicionalM2 = parseNum(e.target.value);
  document.getElementById('f-m2').value = (Number(state.largo||0) * Number(state.ancho||0) + Number(state.adicionalM2||0)).toLocaleString('es-AR', {maximumFractionDigits:2});
  renderPreview();
});
document.getElementById('f-precio-menos15').addEventListener('input', e=>{
  state.precioMenos15 = parseNum(e.target.value);
  renderPreview();
});
document.getElementById('f-precio-menos15').addEventListener('blur', e=>{
  savePreciosBase();
  confirmarCambioCatalogoPrecioBase(e.target, 'precioMenos15', 'Precio por m² para piletas de hasta 15 m²');
});
document.getElementById('f-precio-mas15').addEventListener('input', e=>{
  state.precioMas15 = parseNum(e.target.value);
  renderPreview();
});
document.getElementById('f-precio-mas15').addEventListener('blur', e=>{
  savePreciosBase();
  confirmarCambioCatalogoPrecioBase(e.target, 'precioMas15', 'Precio por m² para piletas de más de 15 m²');
});
document.getElementById('f-precio-instalacion').addEventListener('input', e=>{
  state.precioInstalacion = parseNum(e.target.value);
  renderPreview();
});
document.getElementById('f-precio-instalacion').addEventListener('blur', e=>{
  savePreciosBase();
  confirmarCambioCatalogoPrecioBase(e.target, 'precioInstalacion', 'Costo fijo de instalación');
});
document.getElementById('f-modo-precio').addEventListener('change', e=>{
  state.modoPrecio = e.target.value;
  renderPreview();
});
...
async function buildDocxSections(){
  ...
  // ---- Imágenes: convertir todo a bytes de una sola vez ----
  ...
  // Encabezado con logo sobre fondo de color (tabla de 1 celda, ancho completo)
  ...
  children.push(...docxTitle('Presupuesto de cobertor para piscina'));
  const medidasParts = [`${largo.toLocaleString('es-AR')} m largo × ${ancho.toLocaleString('es-AR')} m ancho = ${espejoM2.toLocaleString('es-AR',{maximumFractionDigits:2})} m²`];
  if(adicionalM2){ medidasParts.push(`Adicional: ${adicionalM2.toLocaleString('es-AR',{maximumFractionDigits:2})} m²`); }
  children.push(docxMetaCard([
    ...
    { label:'Medidas:', value: medidasParts.join(' + ') + ` = ${m2.toLocaleString('es-AR',{maximumFractionDigits:2})} m² a cubrir` },
  ]));
  ...
  if(state.modoPrecio === 'sin'){
    children.push(docxPriceLine('TOTAL', fmt(totalSin), {bold:true, big:true, topRule:true}));
  } else if(state.modoPrecio === 'con'){
    children.push(docxPriceLine('TOTAL (incluye instalación)', fmt(totalCon), {bold:true, big:true, topRule:true}));
  } else {
    children.push(docxPriceLine('TOTAL SIN INSTALACIÓN', fmt(totalSin), {bold:true, big:true, topRule:true}));
    children.push(docxPriceLine('TOTAL CON INSTALACIÓN', fmt(totalCon), {bold:true, big:true}));
  }

  if(incluidosOpt.length){
    children.push(docxSectionTitle('Opcionales'));
    incluidosOpt.forEach(op=>{
      const priceTxt = (op.price===null||op.price===undefined) ? 'No incluye' : fmt(op.price);
      const fotosOp = state.fotosPorOpcional[op.id];
...
async function downloadWord(){
  ...
  try{
    await cargarDocx();
    const nombreArchivo = window.armarNombreArchivo('Cobertor', state.cliente, state.fecha);
...
function imprimirConNombre(){
  ...
  document.title = window.armarNombreArchivo('Cobertor', state.cliente, state.fecha);

(function(){
/* ---------------- STORAGE KEYS ---------------- */
const K_CATALOG = 'presupuesto-cerco-catalogo-opcionales';
const K_LEGAL   = 'presupuesto-cerco-texto-legal';
const K_FOOTER  = 'presupuesto-cerco-footer-empresa';
const K_FOTOS_OPC = 'presupuesto-cerco-fotos-por-opcional'; // metadata liviana; los bytes van en IndexedDB
const K_PRECIOS = 'presupuesto-cerco-precios-base'; // precios base compartidos por todo el equipo (no por presupuesto)

/* ---------------- DEFAULTS ---------------- */
const defaultOptionales = [
  {id:cid(), slug:'porton_reforzado', desc:'Portón de acceso reforzado con cerradura', price:null, included:false},
  {id:cid(), slug:'refuerzo_postes', desc:'Refuerzo de postes en esquinas o desniveles del terreno', price:null, included:false},
  {id:cid(), slug:'lona_premium', desc:'Lona premium (mayor gramaje / color a elección)', price:null, included:false},
];

const defaultLegal = `CERCO PERIMETRAL PARA PISCINA:
...
const DEFAULT_FOTOS_GENERALES_SEED = [
  "/seeds/general-1-dcd4b589.jpg",
  "/seeds/general-2-16872cab.jpg",
  "/seeds/general-3-995e28db.jpg"
];
...
function openFotosDB(){
  ...
  _dbPromise = new Promise((resolve, reject)=>{
    const req = indexedDB.open('PlayaSolCercosDB', 1);
    req.onupgradeneeded = ()=>{
...
function loadCatalog(){
  ...
  try{
    ...
    state.precioSin = precios && precios.precioSin!=null ? precios.precioSin : 63500;
    state.precioCon = precios && precios.precioCon!=null ? precios.precioCon : 79500;
  }catch(e){
    state.precioSin = 63500; state.precioCon = 79500;
  }
}

function savePreciosBase(){
  try{
    localStorage.setItem(K_PRECIOS, JSON.stringify({ precioSin: state.precioSin, precioCon: state.precioCon }));
    flashPreciosSaved();
...
// campo suelto del state, ej. precioSin/precioCon).
...
function quoteToPlainState(){
  return {
    ...
    ml: state.ml, modoPrecio: state.modoPrecio,
    items: state.items,
...
async function aplicarPresupuestoAlState(q){
  ...
  state.ml = q.ml||0; // precioSin/precioCon ya NO se guardan por presupuesto: son precio base compartido (ver loadCatalog/K_PRECIOS)
...
async function newQuote(){
  ...
  state.subtotal = 0;
  state.ml = 0; // precioSin/precioCon quedan como están (precio base compartido, no se reinician por presupuesto)
...
function renderForm(){
  ...
  document.getElementById('f-ml').value = state.ml || '';
  document.getElementById('f-precio-sin').value = state.precioSin;
  document.getElementById('f-precio-sin').dataset.valorInicial = state.precioSin;
  document.getElementById('f-precio-con').value = state.precioCon;
  document.getElementById('f-precio-con').dataset.valorInicial = state.precioCon;
...
// Texto libre (recorrido del cerco) como lista de renglones cortos en vez de un párrafo
// corrido — se divide por saltos de línea y por oración (". "), sin reordenar ni
// inventar contenido. Se reusa igual en el export a Word (docxDimensionCard más abajo).
...
function buildDocumentBody({ forExport=false, photoSrc } = {}){
  const ml = Number(state.ml||0);
  const extras = state.items.reduce((s,i)=>s+Number(i.price||0),0);
  const baseSin = ml * Number(state.precioSin||0);
  const baseCon = ml * Number(state.precioCon||0);
  const totalSin = baseSin + extras;
  const totalCon = baseCon + extras;
  const incluidosOpt = state.opcionales.filter(o=>o.included===true);

  let html = `<div class="doc-title">Presupuesto de cerco perimetral</div>`;
  ...
  html += `<div class="meta-line"><b>Metros lineales a cercar:</b> ${ml.toLocaleString('es-AR')} ml</div>`;
  html += `</div>`;

  if(state.dimension && state.dimension.trim()){
    html += `<div class="section-heading">Detalle del recorrido</div>`;
...
document.getElementById('f-ml').addEventListener('input', e=>{
  state.ml = parseNum(e.target.value);
  renderPreview();
});
document.getElementById('f-precio-sin').addEventListener('input', e=>{
  state.precioSin = parseNum(e.target.value);
  renderPreview();
});
document.getElementById('f-precio-sin').addEventListener('blur', e=>{
  savePreciosBase();
  confirmarCambioCatalogoPrecioBase(e.target, 'precioSin', 'Precio por metro lineal sin instalación');
});
document.getElementById('f-precio-con').addEventListener('input', e=>{
  state.precioCon = parseNum(e.target.value);
  renderPreview();
});
document.getElementById('f-precio-con').addEventListener('blur', e=>{
  savePreciosBase();
  confirmarCambioCatalogoPrecioBase(e.target, 'precioCon', 'Precio por metro lineal con instalación');
});
...
// suave, en vez de líneas sueltas — más fácil de escanear.
...
// usado también en el HTML de pantalla/PDF).
...
async function downloadWord(){
  ...
  try{
    await cargarDocx();
    const nombreArchivo = window.armarNombreArchivo('Cerco', state.cliente, state.fecha);

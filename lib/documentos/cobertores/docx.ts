...
import { FOTOS_REFERENCIA_COBERTORES } from "../fotosSeed";

/**
 * Generador del .docx de Cobertores. Port 1:1 de
 * `buildDocxSections`/`downloadWord` en `public/cobertores-calc.js` — misma
 * estructura que lib/documentos/cercos/docx.ts (comentado ahí con más
 * detalle); acá sólo cambia el título, la ficha de medidas y los defaults.
 */
...
export async function generarDocxCobertores(
  ...
  // Fotos de referencia fijas (Cobertores no modela "cobertor" como opcional
  // de catálogo — es el producto principal — así que no cuelgan de ninguna
  // clave, van siempre). Ver lib/documentos/fotosSeed.ts.
  const seedBytesByUrl: Record<string, Uint8Array> = {};
  for (const f of FOTOS_REFERENCIA_COBERTORES) seedBytesByUrl[f.url] = await urlABytes(f.url);

  function docxFotosReferencia(width = 260) {
    return FOTOS_REFERENCIA_COBERTORES.map(
      (f) =>
        new Paragraph({
  ...
  children.push(
    docxMetaCard([
      ...
      { label: "Medidas:", value: `${medidasTexto.join(" + ")} = ${formatNumero(m2)} m² a cubrir` },
    ])
  );
  ...
  totales.forEach((t, i) => {
    children.push(docxPriceLine(etiquetas[i], formatARS(t), { bold: true, big: true, topRule: i === 0 }));
  });
...
export const TEXTO_LEGAL_POR_DEFECTO = `COBERTOR PARA PISCINA:
...
export const TEXTOS_POR_DEFECTO_COBERTORES: TextosCompartidos = {
